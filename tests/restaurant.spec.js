...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {    
    ...
    // ```
    // esse enunciado que dizer: verifique que a função createMenu() é um objeto e que contém a chave fetchMenu() e que esta chave é uma função.

    const objMenu = createMenu();
    const confirmFetch = createMenu.fetchMenu;
    assert.deepStrictEqual(typeof (createMenu()), 'object');
    assert.deepStrictEqual(Object.keys(objMenu), 'fetchMenu');
    assert.deepStrictEqual(typeof (confirmFetch), 'function');
